...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Halaman Edit User</h1>
    </div>
        ...
        <form method="post" action="/dashboard/users/{{ $user->id }}" class="mb-5" enctype="multipart/form-data">
                ...
                <select class="form-select @error('jobs_id') is-invalid @enderror" aria-label="Default select example"
                    ...
                    @foreach ($jabatans as $jabatan)
                        <option value="{{ $jabatan->id }}"
                            {{ old('jobs_id', $user->jobs_id) == $jabatan->id ? ' selected' : ' ' }}>
                            {{ $jabatan->name }} </option>
                    @endforeach
                </select>
                @error('jobs_id')
            ...
            <div class="mb-3">
                ...
                <input type="password" class="form-control @error('password') is-invalid @enderror" id="password"
                    name="password" value="{{ old('password', $user->password) }}" required autofocus>
                @error('password')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('year') is-invalid @enderror" name="year"
                    id="datepicker" value="{{ old('year', $user->year) }}" required />
                @error('year')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                <select class="form-select @error('countries_id') is-invalid @enderror" aria-label="Default select example"
                    ...
                    @foreach ($countries as $country)
                        <option value="{{ $country->id }}"
                            {{ old('countries_id', $user->countries_id) == $country->id ? ' selected' : ' ' }}>
                            {{ $country->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                @enderror
                <input id="deskripsi" type="hidden" name="deskripsi" value="{{ old('deskripsi', $user->deskripsi) }}">
